...
def getBuyPrice(stock):
    ...
    current_price = snapshot_data[stock]['bid_grp'][2][0]
...
def getSellPrice(stock):
    ...
    current_price = snapshot_data[stock]['offer_grp'][2][0]
...
def tick_data(context,data):
    ...
    #策略一：上午 9:30~9:40：
    for stock, amount in g.hold_position.items():
        #high_px：最高价；
        #last_px：最新成交价；
        #open_px：今开盘价；
        #preclose_px：昨收价；
        ...
        info("--------------Run tick_data----------------" + str(stock) + "，持仓数量：" + str(amount) + "，最新卖二价：" + str(sell_price_2) + "，最新卖三价：" + str(sell_price_3) + "，最高价：" + str(high_px) + ",当前价格：" + str(current_price) + ",最新成交价：" + str(last_px) + ",昨收价：" + str(preclose_px) + ",今开盘价：" + str(open_px) + ",换手率：" + str(turnover_ratio) + ",当前涨跌幅:" + str(px_change_rate) + ", 系统涨跌幅：" + str(sys_px_change_rate), 'info')
        if g.hand_times <= 10:
            if px_change_rate >= 1.8:
                ...
                if current_price < hold_line:
                    ...
                    order(stock, -amount, sell_price_2)
            ...
            #         如果股价跌幅大于-2%，继续持股。
            #         （观察期间，一旦涨跌幅大于-2%，观察期结束；如果涨跌幅再次小于-2%，重新计时观察 10分钟）
            elif px_change_rate > -3 and px_change_rate <= 1:
                info("--------------Run tick_data----------------" + str(stock) + "当前涨跌幅大于-3%，小于1%,持仓，如果直至 2:30 分，股价依然处于此阶段，人工操作卖出", 'info')
...
def removeWatchStock(stock):
    if stock in g.watch_stocks:
        ...
        info("--------------Run removeWatchStock----------------" + str(stock), 'info')
def removeHoldStock(stock):
    if stock in g.hold_position:
        g.hold_position.pop(stock)
        info("--------------Run removeHoldStock----------------" + str(stock), 'info')
...
def get_position_list(context):
    ...
    for position in context.portfolio.positions.values():
        if position.amount != 0:
            position_list[position.sid] = position.amount
